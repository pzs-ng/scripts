...
POE::Session->create(
    package_states => [
        'main' =>
            [qw(_start _stop irc_001 irc_disconnected irc_error irc_socketerr irc_public irc_ctcp_version tick child_stdout sigchld irc_join irc_part irc_kick irc_quit irc_353 irc_366 sigusr1)],
    ], 'heap' => { irc => $irc, zsconfig => YAML::LoadFile($config->{zsconfig}), config => $config }
);
